...
void printString(STR_PRINT_T * ps) {
	...
	while(ps->str[i]) {
		HAL_DMA2D_BlendingStart_IT(&hdma2d, (uint32_t)(ps->font->getChar(ps->str[i])),
									destination, destination,
									ps->font->getWidth(), ps->font->getHeight());
...
void printCroppedString(STR_PRINT_T * ps) {
	...
	while(ps->str[charIndex] && widthRemaining) {
		rightCrop = (widthRemaining < ps->font->getWidth() ? ps->font->getWidth() - widthRemaining : 0);
		if (ps->color) {
...
void printCroppedImg(IMG_PRINT_T * ps) {
	...
	HAL_DMA2D_Start_IT(&hdma2d, source, destination,
					   ps->imgWidth - ps->leftCrop - ps->rightCrop,
					   ps->height);
