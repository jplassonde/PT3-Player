...
TrackSlider::TrackSlider(uint16_t x, uint16_t y, uint16_t xSize, uint16_t ySize) : ScreenElement(x,y, xSize, ySize), font(rezFont18px), pressed(false) {
}
...
void TrackSlider::draw() {
	...
	for (uint16_t row = yPos + ySize/2 - SLIDER_HEIGHT/2; row <= yPos + ySize/2 + SLIDER_HEIGHT/2; row ++) {
		...
		for (uint16_t col = xPos + xSize/2 - SLIDER_LENGTH/2; col <= xPos + xSize/2 + SLIDER_LENGTH/2; col++) {
